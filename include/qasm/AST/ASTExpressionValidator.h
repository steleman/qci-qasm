...
public:
  ...
  bool IsLogicallyNegateType(const ASTExpressionNode* EXN) const {
    ...
    case ASTTypeFunction: {
      if (const ASTFunctionDefinitionNode* FDN =
          dynamic_cast<const ASTFunctionDefinitionNode*>(EXN)) {
        return IsLogicallyNegateType(FDN->GetResultType());
      }
    ...
    case ASTTypeFunctionCall: {
      if (const ASTFunctionCallNode* FCN =
          dynamic_cast<const ASTFunctionCallNode*>(EXN)) {
        if (const ASTFunctionDefinitionNode* FDN =
            dynamic_cast<const ASTFunctionDefinitionNode*>(FCN->GetFunctionDefinition()))
          return IsLogicallyNegateType(FDN);
      }
  ...
  bool IsLogicallyNegateType(const ASTIdentifierRefNode* XId) const {
    assert(XId && "Invalid ASTIdentifierRefNode argument!");
    return IsLogicallyNegateType(XId->GetSymbolType());
  }

  bool IsBitwiseNegateType(ASTType Ty) const {
    switch (Ty) {
    case ASTTypeBool:
    case ASTTypeInt:
    case ASTTypeUInt:
  ...
  bool IsBitwiseNegateType(const ASTExpressionNode* EXN) const {
    assert(EXN && "Invalid ASTExpressionNode argument!");
    ...
    case ASTTypeIdentifierRef: {
      const ASTIdentifierNode* XId = EXN->GetIdentifier();
      assert(XId && "Invalid ASTIdentifierNode contained in an ASTExpressionNode!");
      return IsBitwiseNegateType(XId->GetSymbolType());
    }
      break;
    case ASTTypeUnaryOp: {
      if (const ASTUnaryOpNode* UOP = dynamic_cast<const ASTUnaryOpNode*>(EXN)) {
        ASTType ETy = ASTExpressionEvaluator::Instance().EvaluatesTo(UOP);
        return IsBitwiseNegateType(ETy);
      }
    ...
    case ASTTypeFunction: {
      if (const ASTFunctionDefinitionNode* FDN =
          dynamic_cast<const ASTFunctionDefinitionNode*>(EXN)) {
        return IsBitwiseNegateType(FDN->GetResultType());
      }
    ...
    case ASTTypeFunctionCall: {
      if (const ASTFunctionCallNode* FCN =
        ...
        if (const ASTFunctionDefinitionNode* FDN =
            dynamic_cast<const ASTFunctionDefinitionNode*>(FCN->GetFunctionDefinition()))
          return IsBitwiseNegateType(FDN);
      }
    ...
    return IsBitwiseNegateType(EXN->GetASTType());
  }

  bool IsBitwiseNegateType(const ASTIdentifierNode* XId) const {
    assert(XId && "Invalid ASTIdentifierNode argument!");
    return IsBitwiseNegateType(XId->GetSymbolType());
  }

  bool IsBitwiseNegateType(const ASTIdentifierRefNode* XId) const {
    assert(XId && "Invalid ASTIdentifierRefNode argument!");
    return IsBitwiseNegateType(XId->GetSymbolType());
  }
