...
%union {
  struct {
    ...
    QASM::ASTExitStatementNode* ExitStatementNode;
...
%token <String> TOK_DIRTY TOK_OPAQUE TOK_RESET TOK_EXIT
...
%type <ExitStatementNode>           ExitStmt
...
%type <ExpressionNode>              Expr LogicalNotExpr BitwiseNotExpr
...
ExitStmt
  : TOK_EXIT ';' {
    $$ = ASTProductionFactory::Instance().ProductionRule_2010(GET_TOKEN(1));
  }
  ;
...
BitwiseNotExpr
  : '~' '(' Expr ')' {
    $$ = ASTProductionFactory::Instance().ProductionRule_341(GET_TOKEN(3), $3);
  }
  | '~' Identifier {
    $$ = ASTProductionFactory::Instance().ProductionRule_341(GET_TOKEN(1), $2);
  }
  | '~' UnaryOp {
    $$ = ASTProductionFactory::Instance().ProductionRule_341(GET_TOKEN(1), $2);
  }
