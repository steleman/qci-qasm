...
ASTType ASTExpressionValidator::ResolveOperatorType(const ASTOperatorNode* E,
  ...
  case ASTTypeFunctionCall: {
    if (const ASTOperatorNode* Op = dynamic_cast<const ASTOperatorNode*>(E)) {
      if (const ASTFunctionCallNode* FCN =
          dynamic_cast<const ASTFunctionCallNode*>(Op->GetTargetExpression())) {
  ...
  case ASTTypeFunction: {
    std::stringstream M;
    M << "Nonsensical bitwise negation of a Function Definition.";
...
bool ASTExpressionValidator::Validate(const ASTBinaryOpNode* BOp) const {
  ...
  case ASTTypeOpTy:
    if (const ASTOperatorNode* OPN =
      ...
      LTy = ResolveOperatorType(OPN, LTy);
    else
  ...
  case ASTTypeOpndTy:
    if (const ASTOperandNode* OPN =
      ...
      LTy = ResolveOperandType(OPN, LTy);
    else
      LTy = ASTTypeUndefined;
    break;
